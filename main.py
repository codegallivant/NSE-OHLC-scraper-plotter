...
#Code not in use: (note that its not ordered)
...
csv_folder_path = os.path.join(original_working_path, "ScriptCSVs")
if os.path.isdir(csv_folder_path) is False:
    os.mkdir(csv_folder_path, 0o666)

Processing CSVs & filling data:
os.chdir(csv_folder_path)
    if os.path.isfile(os.path.join(csv_folder_path, f"{code}{str(date.date())}.csv")) is False:
    file_url = f"https://www1.nseindia.com/content/historical/EQUITIES/{date.strftime('%Y')}/{(date.strftime('%b')).upper()}/cm{date.strftime('%d')}{(date.strftime('%b')).upper()}{date.strftime('%Y')}bhav.csv.zip" 
    headers=requests.head(file_url).headers
    downloadable = 'attachment' in headers.get('Content-Disposition', '')
    if downloadable:
        df = pandas.read_csv(file_url, sep=",")
        print(f"Accessing CSV file ({file_url})...") 
        data = df[df["SYMBOL"] == code]
        print(data)
        opening.append(data.iat[0,3])
    high.append(data.iat[0,4])
    low.append(data.iat[0,5])
    closing.append(data.iat[0,6])
    else:
        opening.append()


        df.to_csv(f"{code}{str(date.date())}.csv")
    r = requests.get(file_url, stream = True)
    with open(f"{code}{str(date.date())}.csv","wb") as csv:
...
os.chdir(original_working_path)
    ...
    # '({},{})'.format(datetime.fromordinal(sel.target[0]), "%.3f" % float(sel.target[1]) )))
